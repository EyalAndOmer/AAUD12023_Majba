...
namespace ds::tests
{
    ...
    protected:
        void test() override
        {
            ...
            const auto matrix = adt::Matrix<int>(d1, d2);
    ...
    protected:
        void test() override
        {
            const auto d1 = adt::Dimension(4, 5);
            const auto d2 = adt::Dimension(3, 10);
            auto matrix = adt::Matrix<int>(d1, d2);
    ...
    protected:
        void test() override
        {
            const auto d1 = adt::Dimension(4, 5);
            const auto d2 = adt::Dimension(3, 10);
            auto matrix1 = adt::Matrix<int>(d1, d2);
            for (long long i = d1.getBase(); i < d1.getBase() + static_cast<long long>(d1.getSize()); ++i)
            {
                for (long long j = d2.getBase(); j < d2.getBase() + static_cast<long long>(d2.getSize()); ++j)
                {
            ...
            auto matrix3 = adt::Matrix<int>(d1, d2);
            ...
            this->assert_throws([&]()
                {
                    auto matrix4 = adt::Matrix<int>(adt::Dimension(d1.getBase() + 1, d1.getSize() - 1), d2);
                    matrix4.assign(matrix3);
                });

            this->assert_throws([&]()
                {
                    auto matrix4 = adt::Matrix<int>(d1, adt::Dimension(d2.getBase() + 1, d2.getSize() - 1));
                    matrix4.assign(matrix3);
                });
        }
    };
