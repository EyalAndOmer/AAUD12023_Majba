...
namespace ds::amt
{
    ...
    // Ked pouzijem auto na navrat. hodnotu metody tak pouzijem const -> Navrat nakonci, kedze je BlockType definovany
    // v ImplicitSequence, ku ktorej pri navrat hodnote metode nemam este pristup (cita sa to zlava doprava)

    template<typename DataType>
    auto ImplicitSequence<DataType>::access(size_t index) const -> BlockType*
    {
        // Nemusime kontrolovat ci je mensi ako 0, lebo do size_t sa neda priradit zaporna hodnota
    ...
    // Prefixovy iterator, rychlejsi ako postfix, nevytvaram lokalnu premennu
    template <typename DataType>
    ...
    // Postfixovy operator, urcuje to dummy parameter int, ktory sluzi len compilatoru, aby vedel aky je to druh iteratora
    template <typename DataType>
