...
namespace ds::amt {
	...
	{
		if (node != nullptr) {
			BlockType* left_son = this->accessLeftSon(*node);
			processInOrder(left_son, operation);

			operation(node);

			BlockType* right_son = this->accessRightSon(*node);
			processInOrder(right_son, operation);
		}
	}
	...
	// Typename nam zabezpecuje kompilaciu programu
	// C++ by bez typename nevnimalo Hierarchy<BlockType>::PreOrderHierarchyIterator ako typ, ale ako premennu alebo nazov funkcie
	template<typename BlockType>
